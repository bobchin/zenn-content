...
@asm_pio(sideset_init=PIO.OUT_HIGH, out_init=PIO.OUT_HIGH, out_shiftdir=PIO.SHIFT_RIGHT, autopull=True, pull_thresh=8)
def uart_tx():
  # TX FIFO から OSR にプル
  pull()                      # 1サイクル
  # スタートビット(1bit の Low=side(0))
  # set の 7 はデータビットの7ビットを7回ループするための値
  set(x, 7)     .side(0)  [7] # 1 + 7 = 8サイクル
  label("bitloop")
  # データビット(LSBから１ビットずつ７ビット分)
  # 7 + 1 = 8サイクルでループ
  out(pins, 1)            [6] # 1 + 6 = 7サイクル
  jmp(x_dec, "bitloop")       # 1サイクル
  # ストップビット(1bit の High=side(1))
  nop()         .side(1)  [6] # 1 + 6 = 7サイクル。ループして pull() するので 8サイクルになる。

# PIO 処理開始
sm = StateMachine(0, uart_tx, freq=UART_BAUD*8, sideset_base=PIO_TX_PIN, out_base=PIO_TX_PIN)
sm.active(1)
for c in "123\n":
  sm.put(ord(c))
  utime.sleep(0.1)
...
@asm_pio(in_shiftdir=PIO.SHIFT_RIGHT, fifo_join=PIO.JOIN_RX, autopush=True, push_thresh=8)
def uart_rx():
  # スタートビットを待つ
  wait(0, pin, 0) # 1サイクル
  # ループカウンタをセット。
  # 最初のデータビットまで待つ。スタートビット8bit + 4bit(データの真ん中のタイミングを読み込むため)
